        ...
        <div class="card-header">
            <div class="row">
                ...
                <div class="col-lg-4 text-end mb-3">
                    <a href="{{ route('admin-ppdb-cara-pendaftaran.create') }}" class="btn btn-success"><i
                            class='bx bx-plus-circle' style="font-size: 1.5em"></i> Tambah Cara Pendaftaran
                    </a>
                </div>
        ...
        <div class="table-responsive text-nowrap p-3">
            <table class="table table-striped datatable">
                ...
                <tbody class="table-border-bottom-0">
                    @foreach ($cara_pendaftaran as $index => $item)
                        <tr>
                            <th class="text-center" scope="row">{{ $index + 1 }}</th>
                            <td>{{ $item->step }}</td>
                            <td>{{ $item->nama_step }}</td>
                            <td>{{ $item->logo }}</td>
                            <td> {!! html_entity_decode($item->deskripsi) !!}</td>
                            <td>
                                <a href="{{ route('admin-ppdb-cara-pendaftaran.edit', $item->id) }}">
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        <form action="{{ route('admin-ppdb-cara-pendaftaran.destroy', $item->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
