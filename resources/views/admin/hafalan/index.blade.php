        ...
        <div class="card-header">
            <div class="row">
                ...
                <div class="col-lg-4 text-end mb-3">
                    <a href="{{ route('admin-hafalan-santri.create') }}" class="btn btn-success"><i class='bx bx-plus-circle'
                ...
                <tbody class="table-border-bottom-0">
                    @foreach ($hafalan as $index => $item)
                        <tr>
                            ...
                            <td>{{ $item->surat }}</td>
                            <td>{{ $item->ayat }}</td>
                            <td>{{ $item->total_hafalan }}</td>
                            <td>
                                <a href="{{ route('admin-hafalan-santri.edit', $item->id) }}">
                        ...
                        {{-- Modal Delete --}}
                        <div class="modal fade" id="basicModal{{ $item->id_santri }}" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        <form action="{{ route('admin-hafalan-santri.destroy', $item->id_santri) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
